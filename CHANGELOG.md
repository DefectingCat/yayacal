...
- 五线谱练习工具：工具页新增「五线谱练习」入口，认识钢琴高音谱表。教学页含五线谱基础讲解、线/间结构图解、音名/唱名/简谱对照、可点按的 do~si 音阶阶梯与钢琴键盘（联动查看音名/唱名/谱面位置）、逐步引导的「互动小课堂」（点音符作答、答错报音名并给位置提示）及位置口诀；练习页支持「看谱认唱名」与「听名找位置」双向四选一练习，作答即时反馈（答对绿、答错红并标出正确答案），带已答/正确率/连击统计，全交互配有克制的过渡动画。
- 五线谱教学扩充：教学页新增「高音与低音」探索器（三个八度键盘与高音谱表、详情行三方联动，讲解八度与简谱高低音点）、完整五线谱音位图（C4~C6 十五个自然音逐一标记、点按查看名称与位置）与「大谱表」图解（高音/低音谱号示意、中央 C 共享加线、左右手分工）；音符详情补充简谱记法与音区名称，并修正下加间位置描述的序号。
