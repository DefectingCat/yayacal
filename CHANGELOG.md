...
- 五线谱练习工具：工具页新增「五线谱练习」入口，包含完整的系统化五线谱教学与互动练习两大模块。
  - 练习模式：支持「看谱选唱名（附简谱数字）」与「看唱名找谱面位置」双向互动选择题，提供即时高亮反馈、连对计数、题目提示、发光卡片动效与震动反馈。
  - 谱表与难度定制：支持高音谱表、低音谱表、大谱表双谱混合三种谱表模式，以及基础音域（高音 C4~G5 / 低音 F2~C4）与全音域（高音 C3~C6 / 低音 C2~E4 / 大谱表 C2~C6）自由切换。
- 五线谱系统化交互教学体系：
  - 6 大主题胶囊导航：识谱入门、进阶乐理、符号速查、读谱技巧、指法调号、互动小课堂，支持平滑滑动与切换动效。
  - 识谱入门与探索器：含线/间结构图解、大谱表左右手分工与中央 C 共享加线图解、C2~C6 宽音域钢琴键盘自适应跟随与谱表视窗探索器、C4~C6 完整音位图及 do~si 阶梯音阶。
  - 进阶乐理与时值树：音符与休止符完整时值树（全/二分/四分/八分/十六分），附点音符与三连音节拍原理图解。
  - 符号速查字典：覆盖常见拍号、速度术语（Largo 到 Presto）、力度记号（pp 到 ff 及渐强渐弱）、演奏与踏板记号、变音与反复记号（D.C./D.S./Fine/Coda 等）。
  - 读谱技巧与地标音：三 C 定位法、五度地标音、级进/跳进速读法则。
  - 指法与调号速记：左右手五指编号对照、升降号调号判定口诀表、八度记号（8va/8vb）与和弦标记。
  - 互动小课堂：分步式互动答题，即时点击谱面音符判定，答错自动提供音名与线间位置提示。

### Changed

- 谱号高精度矢量重构：大谱表高音谱号（G 谱号）与低音谱号（F 谱号）全面改用 LilyPond Feta 真实字形矢量路径渲染，低音谱号双点精准夹住第四线（F3），高音谱号螺旋内环精准压第二线（G4），优化大谱表整体比例与对齐。
- 交互与过渡动效优化：全面优化教学卡片展开/收起、难度模式切换及胶囊导航的平滑过渡动画。

### Fixed

- 互动小课堂判定：修复由于 Compose `pointerInput` 闭包引发的目标音状态更新滞后及点击判定错位问题。
- 探索器视窗适配：修复高音低音探索器低八度音符在紧凑视窗下被裁切的问题。
- 练习选项去重：修复练习题唱名选项偶发重复的问题。

## [1.7.0] - 2026-08-17

### Added

- 纪念日与岁时档案：新增纪念日工具页，展示「在一起」累计天数（自 2025-11-04 起）与里程碑节点倒数（100/365/520/1000/1314 天及整周年），以及鸭鸭生日（公历 9 月 4 日）与小狗生日（农历正月廿一）的逐年倒数；支持点击进入纪念日期详情与岁时档案内页，页面采用 Material 3 Expressive Bento 布局与级联入场动效。
- 节日与纪念问候遮罩系统：
  - 七夕问候遮罩：七夕当天（农历七月初七，经 tyme4kt 判定并排除闰七月）冷启动展示节日专属壁纸与「七夕快乐！」艺术字（站酷快乐体子集，SIL OFL 授权）入场动效。
  - 生日问候遮罩：公历 9 月 4 日冷启动展示线条小狗钞票壁纸与「生日快乐！」艺术字，金色皇冠轻落与文字浮起级联入场。
  - 表白日问候遮罩：公历 11 月 4 日冷启动展示专属壁纸与「今天，是表白日呀」暖心问候。
  - 所有问候遮罩均支持点击任意处或返回键以 300ms 平滑淡出退出，非触发日期完全不出现。

### Changed

- 依赖版本对齐：升级 `sketch` 图像加载库至 4.6.0，与 `zoomimage` 传递依赖严格对齐。

### Fixed

- 线条小狗动图静帧化：修复由于 sketch 模块版本错位导致 AnimatedWebp 解码器工厂不可见、动图静帧化停留在首帧的问题。
- 日期记录器视图切换：限制共享元素动画图层在记录网格区域内绘制，避免滚动后切换布局时记录卡片越过筛选栏并遮挡顶部状态栏和标题栏。
