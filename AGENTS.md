...
Root `Makefile` wraps the common tasks — bare `make` builds the release APK; `make help` lists all targets (`release` / `build` / `install` / `test` / `fmt` / `check` / `clean` / `profile`); `make test T=CalendarUtilsTest` runs a single test class.
